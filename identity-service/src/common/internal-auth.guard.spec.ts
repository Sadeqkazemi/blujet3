...
describe('identity internal service identity', () => {
  const token = 'fixture-identity-token-at-least-32-characters';
  const getOrThrow = jest.fn();
  const reflector = new Reflector();
  const guard = new InternalAuthGuard(reflector, {
    getOrThrow,
  ...
  it('accepts only its configured service token', () => {
    ...
    expect(getOrThrow).toHaveBeenCalledWith('IDENTITY_INTERNAL_TOKEN');
  });
  it.each([undefined, '', 'wrong', 'fixture-other-service-token', [token]])(
    'rejects missing, incorrect or ambiguous credentials (%#)',
    (supplied) => {
      expect(() => guard.canActivate(context(supplied))).toThrow(UnauthorizedException);
    },
  );
  it('rejects the old token after configured token replacement', () => {
    getOrThrow.mockReturnValue('fixture-replacement-token-at-least-32-characters');
    expect(() => guard.canActivate(context(token))).toThrow(UnauthorizedException);
    expect(
      guard.canActivate(context('fixture-replacement-token-at-least-32-characters')),
    ).toBe(true);
  });
  ...
  it('bypasses credentials only for explicitly public endpoint metadata', () => {
    const health = () => undefined;
    Reflect.defineMetadata(PUBLIC_ENDPOINT, true, health);
    expect(guard.canActivate(context(undefined, health))).toBe(true);
