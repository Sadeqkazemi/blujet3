...
describe('AgencyKafkaRuntime', () => {
  ...
  const enabled: AgencyKafkaConsumerConfig = {
    enabled: true,
    requireSchemaId: false,
    topic: 'blujet.events.v1',
    fromBeginning: true,
    maxBytes: 4096,
    client: { clientId: 'agency', brokers: ['localhost:9092'] },
    consumer: {
      groupId: 'agency-v1',
  ...
  function runtime(
    ...
    kafkaClient: AgencyKafkaRuntimeClient | null,
  ): AgencyKafkaRuntime {
    return new AgencyKafkaRuntime(
      config,
      kafkaClient,
  ...
  it.each(['connect', 'subscribe', 'run'] as const)(
    ...
    async (stage) => {
      ...
      await expect(worker.onApplicationBootstrap()).rejects.toThrow(
        'Agency Kafka consumer startup failed',
      );
  ...
  it('sanitizes shutdown failure and still disconnects', async () => {
    ...
    await expect(worker.onApplicationShutdown()).rejects.toThrow(
      'Agency Kafka consumer shutdown failed',
    );
    expect(kafkaClient.disconnect).toHaveBeenCalledTimes(1);
    expect(logger.error).toHaveBeenCalledWith(
      'Agency Kafka consumer shutdown failed',
  ...
  it('records and propagates a content-free processing failure', async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith('Agency Kafka processing failed');
    expect(worker.getStatus()).toMatchObject({
      state: 'running',
