...
describe('loyalty internal service identity', () => {
  const token = 'fixture-loyalty-token-at-least-32-characters';
  ...
  it('accepts only its configured service token', () => {
    ...
    expect(getOrThrow).toHaveBeenCalledWith('LOYALTY_INTERNAL_TOKEN');
  });
