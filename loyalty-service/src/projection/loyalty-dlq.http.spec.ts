...
import { LOYALTY_DLQ_CONFIG } from '../loyalty-dlq.config';
...
const token = 'loyalty-operator-token-at-least-32-characters';
...
@Module({
  ...
  providers: [
    LoyaltyDlqAuthGuard,
    { provide: LoyaltyDlqStore, useValue: dlq },
    {
      provide: LOYALTY_DLQ_CONFIG,
      useValue: { enabled: true, maxAttempts: 3, operatorToken: token },
    },
  ],
})
class LoyaltyDlqHttpTestModule {}

describe('LoyaltyDlqController HTTP boundary', () => {
  let app: INestApplication<App>;

  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      imports: [LoyaltyDlqHttpTestModule],
    }).compile();
  ...
  it('validates and audits retry and skip decisions', async () => {
    await request(app.getHttpServer())
      .post(`/internal/v1/loyalty/dlq/${recordId}/skip`)
      .set('X-Internal-Token', token)
    ...
    await request(app.getHttpServer())
      ...
      .expect(201);
    await request(app.getHttpServer())
      .post(`/internal/v1/loyalty/dlq/${recordId}/retry`)
      .set('X-Internal-Token', token)
  ...
  it('rejects invalid and missing quarantine identifiers', async () => {
    await request(app.getHttpServer())
      .post('/internal/v1/loyalty/dlq/not-a-uuid/retry')
      .set('X-Internal-Token', token)
      .send({ operatorId: 'operator-2', reason: 'dependency restored' })
      .expect(400);
