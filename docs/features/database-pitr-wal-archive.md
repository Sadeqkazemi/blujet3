...
- [x] The production PostgreSQL service enables `wal_level=replica`,
  `archive_mode=on`, a bounded `archive_timeout`, and a fail-closed
  `archive_command` — `backend/src/production-artifacts.spec.ts`
- [x] WAL files are written to a volume separate from the primary `PGDATA`
...
- [x] `scripts/backup-base-pitr.sh` creates a plain physical base backup with
...
- [x] A failed base backup never prunes the WAL archive; successful retention
  cleanup keeps WAL from the oldest retained base backup — shell implementation
  and `backend/src/production-artifacts.spec.ts`
- [x] CI starts PostgreSQL 16 with the production archive helper, takes a base
...
- [x] The required backup/restore proof jobs are dependencies of `CI gate`, so
  a skipped or failed selected proof cannot be hidden by a green gate —
  `backend/src/production-artifacts.spec.ts`
- [x] The runbook documents base-backup scheduling, health checks, retention,
  destructive PITR recovery, and the unresolved off-site storage gate —
  `docs/RUNBOOK.md`
- [x] No deployment, production cron change, restore, or merge occurred in this
  implementation phase; merge remains behind separate owner approval.
