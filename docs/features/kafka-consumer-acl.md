...
- [x] Explicit exact-topic Read and exact-group Read grants permit consumption of
  an exact canonical event by the restricted consumer (same spec: consumes after
  granting only literal-topic Read and group Read).
...
- [ ] Owner-approved CI on Linux/ARM real Kafka before merge. No deployment.

Files: ackend/test/kafka/local-kafka-security.ts,
ackend/test/kafka/kafka-security.kafka-spec.ts, API/schema notes, roadmap and
