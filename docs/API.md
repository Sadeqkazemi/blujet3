...
(`docs/features/microservices-phase-6-agency-kafka-checkpoints.md`).
...
SHA-256 is supplied separately and streamed without loading the dump into
memory. The opt-in Compose transfer profile mounts that artifact read-only at
a fixed container path. Evidence is source/target counts, two
...
existing internal cartable columns only and no access to projection receipts,
Kafka checkpoints or the failure quarantine. Kafka, dual-write, reader
cutover and deployment remain disabled
...
When at least 12 hours remain, a request made within 24 hours of purchase uses
the 30% bracket before the departure-time brackets. After a request is
...
| GET    | `/it/backups/schedule` | Reports the enforced nightly server cron (`scripts/backup-db.sh`, 03:00), seven-day retention, and honest capability status (`fileBackup`, `cloudStorage`). Also returns `lastSuccessfulBackupAt` and `lastSuccessfulBackupFile`, or `null` when no successful dump is recorded. It does not claim that an operator-managed cron or off-site storage is active beyond this evidence. |
