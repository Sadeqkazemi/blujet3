...
`loyalty.kafka_consumer_checkpoints`: `consumerGroup varchar(128)`,
`topic varchar(249)`, non-negative `partition int`, non-negative
`nextOffset bigint`, nullable non-negative `highWatermark bigint` and
`updatedAt timestamptz(3)`. The composite primary key is
`(consumerGroup, topic, partition)`.
...
`ops.cartable_projection_event_receipts`. Other databases lose PUBLIC CONNECT;
the owner keeps an explicit GRANT. PUBLIC, Core schemas, receipt UPDATE,
DELETE, TRUNCATE,
...
- `RefundPenaltyRule { id, minHoursBeforeDeparture, penaltyPct, labelFa }` — ⚑ the customer panel's four-bracket engine is the server-side source of truth: >72h→30٪, 24–72h→50٪, 12–<24h→70٪, <12h or after departure→100٪ (غیرقابل استرداد), per the approved BluJet cancellation policy. When at least 12h remains, the first 24h after purchase uses the 30٪ bracket. Penalty is computed server-side at request creation; the static settings editor is dead UI and is not built.
