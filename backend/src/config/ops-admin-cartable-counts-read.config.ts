export function opsAdminCartableCountsReadConfig(
  input: Record<string, unknown>,
) {
  const flag = input.OPS_ADMIN_CARTABLE_COUNTS_READ_ENABLED ?? 'false';
  if (flag === 'false') return { enabled: false as const };
  if (flag !== 'true')
    throw new Error('Invalid Ops/Admin cartable counts read flag');
  if (Intl.DateTimeFormat().resolvedOptions().timeZone !== 'UTC')
    throw new Error(
      'Ops/Admin cartable counts read integration requires a UTC runtime (TZ=UTC)',
    );
