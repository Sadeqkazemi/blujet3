...
export const REPORTING_PROJECTION_RUNTIME_ROLE =
  'blujet_reporting_projection_runtime';
export const REPORTING_DATABASE_NAME_PATTERN =
  /^blujet_reporting(?:_[A-Za-z0-9_]+)?$/;
export const REPORTING_MIN_SERVER_VERSION = 160000;

export const REPORTING_RUNTIME_GRANTS = [
  {
...
export interface ReportingRuntimeRoleSqlClient {
  query(
    text: string,
...
export function validateReportingRuntimePassword(password: string): void {
  if (!/^[A-Za-z0-9_-]{32,128}$/.test(password)) {
    throw new Error(
      'REPORTING_PROJECTION_RUNTIME_PASSWORD must be 32-128 URL-safe characters',
    );
  }
}

export function validateReportingDatabaseName(databaseName: string): void {
...
export function parseReportingOwnerUrl(ownerUrl: string): {
  databaseName: string;
  username: string;
} {
  let parsed: URL;
  try {
    ...
    throw new Error('REPORTING_DATABASE_OWNER_URL must be PostgreSQL');
  }
  if (parsed.protocol !== 'postgresql:' && parsed.protocol !== 'postgres:') {
    throw new Error('REPORTING_DATABASE_OWNER_URL must be PostgreSQL');
  }
  const username = decodeURIComponent(parsed.username);
  if (!username || username === REPORTING_PROJECTION_RUNTIME_ROLE) {
    throw new Error(
      'Reporting database owner must differ from the runtime role',
    );
  }
...
function expectedTablesSql(): string {
  return REPORTING_RUNTIME_GRANTS.map(({ table }) => literal(table)).join(', ');
}

function allowedPrivilegesSql(): string {
  const grants: readonly ReportingRuntimeGrant[] = REPORTING_RUNTIME_GRANTS;
  return grants
...
async function assertReportingSchema(
  client: ReportingRuntimeRoleSqlClient,
): Promise<number> {
  ...
  const actual = result.rows.map((row) => {
    if (typeof row.name !== 'string') {
      throw new Error('Reporting runtime relation contract does not match');
    }
    return row.name;
  });
  const expected = REPORTING_RUNTIME_GRANTS.map(({ table }) => table).sort();
  if (
...
async function verifyReportingRuntimeRole(
  client: ReportingRuntimeRoleSqlClient,
  databaseName: string,
): Promise<void> {
  ...
  const result = await client.query(`WITH role_state AS (
      ...
      FROM pg_roles WHERE rolname = '${REPORTING_PROJECTION_RUNTIME_ROLE}'
    ...
    SELECT
      ...
      has_database_privilege(
        '${REPORTING_PROJECTION_RUNTIME_ROLE}', ${literal(databaseName)}, 'CONNECT'
      ) AND NOT has_database_privilege(
        '${REPORTING_PROJECTION_RUNTIME_ROLE}', ${literal(databaseName)}, 'CREATE,TEMP'
      ) AS "databaseAccess",
      has_schema_privilege(
        '${REPORTING_PROJECTION_RUNTIME_ROLE}', 'reporting', 'USAGE'
      ) AND NOT has_schema_privilege(
        '${REPORTING_PROJECTION_RUNTIME_ROLE}', 'reporting', 'CREATE'
      ) AS "schemaAccess",
      NOT EXISTS (
        SELECT 1 FROM allowed
        LEFT JOIN reporting_relations relation
          ON relation.relname = allowed.table_name AND relation.relkind IN ('r', 'p')
        WHERE relation.oid IS NULL OR NOT has_table_privilege(
          '${REPORTING_PROJECTION_RUNTIME_ROLE}', relation.oid,
        ...
        WHERE relation.relkind IN ('r', 'p') AND (
          ...
          OR EXISTS (
            ...
            WHERE has_table_privilege(
              '${REPORTING_PROJECTION_RUNTIME_ROLE}', relation.oid, privilege
            )
          )
          OR relation.relname = 'core_itinerary_event_receipts' AND
            has_any_column_privilege(
              '${REPORTING_PROJECTION_RUNTIME_ROLE}', relation.oid, 'UPDATE'
            )
      ...
      NOT EXISTS (
        ...
        WHERE database.datallowconn AND NOT database.datistemplate
          ...
          AND has_database_privilege(
            '${REPORTING_PROJECTION_RUNTIME_ROLE}', database.oid, 'CONNECT'
          )
      ) AS "noForeignConnect"`);
  ...
  if (
    ...
    throw new Error('Reporting projection runtime role verification failed');
  }
}

export async function provisionReportingProjectionRuntimeRole(
  client: ReportingRuntimeRoleSqlClient,
  password: string,
  expectedDatabaseName?: string,
): Promise<{ status: 'PASS'; role: string; relationCount: number }> {
  validateReportingRuntimePassword(password);
  const role = identifier(REPORTING_PROJECTION_RUNTIME_ROLE);
  await client.query('BEGIN');
  try {
    await client.query(`DO $$ BEGIN
      IF NOT EXISTS (
        SELECT 1 FROM pg_roles
        WHERE rolname = '${REPORTING_PROJECTION_RUNTIME_ROLE}'
    ...
    if (typeof statement !== 'string') {
      throw new Error('PostgreSQL could not prepare the Reporting credential');
    }
    ...
    await client.query(`DO $$ DECLARE parent_role record; BEGIN
      FOR parent_role IN
        ...
        WHERE member.rolname = '${REPORTING_PROJECTION_RUNTIME_ROLE}'
    ...
    if (version < REPORTING_MIN_SERVER_VERSION) {
      throw new Error('Reporting projection runtime requires PostgreSQL 16');
    }
    if (
    ...
    await client.query(`DO $$ DECLARE target record; BEGIN
      FOR target IN SELECT datname FROM pg_database
        WHERE datallowconn AND NOT datistemplate
          AND datname <> current_database()
      LOOP
        EXECUTE format('REVOKE CONNECT ON DATABASE %I FROM PUBLIC', target.datname);
        EXECUTE format('REVOKE CONNECT ON DATABASE %I FROM ${role}', target.datname);
      END LOOP;
    END $$`);
    ...
    await client.query(`REVOKE ALL ON SCHEMA reporting FROM ${role}`);
    await client.query(`GRANT USAGE ON SCHEMA reporting TO ${role}`);
    await client.query(
      `REVOKE ALL ON ALL TABLES IN SCHEMA reporting FROM ${role}`,
    );
    await client.query(
      `REVOKE ALL ON ALL SEQUENCES IN SCHEMA reporting FROM ${role}`,
    );
    await client.query(`ALTER DEFAULT PRIVILEGES IN SCHEMA reporting
      REVOKE ALL ON TABLES FROM ${role}`);
    await client.query(`ALTER DEFAULT PRIVILEGES IN SCHEMA reporting
      REVOKE ALL ON SEQUENCES FROM ${role}`);
    for (const grant of REPORTING_RUNTIME_GRANTS) {
      await client.query(
        `GRANT ${grant.privileges.join(', ')} ON reporting.${identifier(grant.table)} TO ${role}`,
      );
    }
    ...
    await verifyReportingRuntimeRole(client, databaseName);
...
async function main(): Promise<void> {
  const ownerUrl = process.env.REPORTING_DATABASE_OWNER_URL;
  const password = process.env.REPORTING_PROJECTION_RUNTIME_PASSWORD;
  if (!ownerUrl || !password) {
    throw new Error(
      'REPORTING_DATABASE_OWNER_URL and REPORTING_PROJECTION_RUNTIME_PASSWORD are required',
    );
  }
  const parsed = parseReportingOwnerUrl(ownerUrl);
  const client = new Client({ connectionString: ownerUrl });
  await client.connect();
  try {
    const report = await provisionReportingProjectionRuntimeRole(
      client,
      password,
      parsed.databaseName,
    );
...
if (require.main === module) {
  void main().catch(() => {
    process.stderr.write(
      'Reporting projection runtime role provisioning failed\n',
    );
    process.exitCode = 1;
  });
}
