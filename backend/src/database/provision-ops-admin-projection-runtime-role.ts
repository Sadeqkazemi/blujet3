...
export const OPS_ADMIN_PROJECTION_RUNTIME_SCHEMA = 'ops';
export const OPS_ADMIN_PROJECTION_RUNTIME_GRANTS = [
  {
    table: 'cartable_tasks',
    privileges: 'SELECT, INSERT, UPDATE',
    forbidUpdate: false,
  },
  {
    table: 'cartable_projection_event_receipts',
...
export const OPS_ADMIN_PROJECTION_DATABASE_NAME_PATTERN =
  /^blujet_ops_admin(_[A-Za-z0-9_]+)?$/;
export const OPS_ADMIN_PROJECTION_MIN_SERVER_VERSION = 160000;
...
export function validateOpsAdminProjectionRuntimePassword(
  password: string,
): void {
  if (!/^[A-Za-z0-9_-]{32,128}$/.test(password)) {
    throw new Error(
      'OPS_ADMIN_PROJECTION_RUNTIME_PASSWORD must be 32-128 URL-safe characters',
    );
  }
}

export function validateOpsAdminProjectionDatabaseName(
  databaseName: string,
): void {
  if (!OPS_ADMIN_PROJECTION_DATABASE_NAME_PATTERN.test(databaseName)) {
    throw new Error(
...
export function parseOpsAdminProjectionOwnerUrl(ownerUrl: string): {
  databaseName: string;
  username: string;
} {
  let parsed: URL;
  try {
    ...
    throw new Error(
      'OPS_ADMIN_PROJECTION_DATABASE_OWNER_URL must be PostgreSQL',
    );
  }
...
export async function provisionOpsAdminProjectionRuntimeRole(
  client: RuntimeRoleSqlClient,
  password: string,
  expectedDatabaseName?: string,
): Promise<{ status: 'PASS'; role: string; relationCount: number }> {
  validateOpsAdminProjectionRuntimePassword(password);
  const role = identifier(OPS_ADMIN_PROJECTION_RUNTIME_ROLE);
  const schema = identifier(OPS_ADMIN_PROJECTION_RUNTIME_SCHEMA);
  const allowedTables =
    OPS_ADMIN_PROJECTION_RUNTIME_TABLES.map(literal).join(', ');
  const allowedGrantValues = OPS_ADMIN_PROJECTION_RUNTIME_GRANTS.map(
    (grant) =>
  ...
  try {
    await client.query(`DO $$ BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = '${OPS_ADMIN_PROJECTION_RUNTIME_ROLE}') THEN
    ...
    if (typeof passwordStatement !== 'string') {
      throw new Error(
        'PostgreSQL could not prepare the Ops/Admin projection runtime credential',
      );
    }
    ...
    await client.query(`DO $$ DECLARE parent_role record; BEGIN
      FOR parent_role IN
        ...
        WHERE member.rolname = '${OPS_ADMIN_PROJECTION_RUNTIME_ROLE}'
      LOOP
    ...
    if (version < OPS_ADMIN_PROJECTION_MIN_SERVER_VERSION) {
      throw new Error('Ops/Admin projection runtime requires PostgreSQL 16');
    }
    validateOpsAdminProjectionDatabaseName(databaseName);
    if (
    ...
    const required = await client.query(
      ...
      WHERE n.nspname = '${OPS_ADMIN_PROJECTION_RUNTIME_SCHEMA}'
        AND c.relkind IN ('r', 'p')
        AND c.relname = ANY(ARRAY[${allowedTables}])`,
    );
    if (required.rows.length !== OPS_ADMIN_PROJECTION_RUNTIME_TABLES.length) {
      throw new Error('Ops/Admin projection runtime relations are missing');
    }
    ...
    const verification = await client.query(`WITH role_state AS (
      ...
      FROM pg_roles WHERE rolname = '${OPS_ADMIN_PROJECTION_RUNTIME_ROLE}'
    ), owned AS (
      ...
      FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace
      WHERE n.nspname NOT IN ('${OPS_ADMIN_PROJECTION_RUNTIME_SCHEMA}', 'pg_catalog', 'information_schema')
    ...
    SELECT
      ...
      NOT EXISTS (
        ...
        LEFT JOIN ops_relations relation ON relation.relname = allowed.relname
          AND relation.relkind IN ('r', 'p')
        WHERE relation.oid IS NULL
          OR NOT has_table_privilege(
            '${OPS_ADMIN_PROJECTION_RUNTIME_ROLE}', relation.oid, allowed.required
          )
          OR (
            allowed.forbid_update
            AND has_table_privilege(
              '${OPS_ADMIN_PROJECTION_RUNTIME_ROLE}', relation.oid, 'UPDATE'
            )
          )
      ) AS "ownAccess",
      NOT EXISTS (
        ...
        WHERE (relation.relkind IN ('r', 'p') AND (
          (
            ...
            AND (
              has_table_privilege(
                '${OPS_ADMIN_PROJECTION_RUNTIME_ROLE}', relation.oid, 'DELETE'
              )
              OR has_table_privilege(
                '${OPS_ADMIN_PROJECTION_RUNTIME_ROLE}', relation.oid, 'TRUNCATE'
              )
              OR has_table_privilege(
                '${OPS_ADMIN_PROJECTION_RUNTIME_ROLE}', relation.oid, 'REFERENCES'
              )
              OR has_table_privilege(
                '${OPS_ADMIN_PROJECTION_RUNTIME_ROLE}', relation.oid, 'TRIGGER'
              )
              OR (
                allowed.forbid_update
                AND has_table_privilege(
            ...
            AND (
              has_any_column_privilege(
                '${OPS_ADMIN_PROJECTION_RUNTIME_ROLE}', relation.oid,
          ...
          '${OPS_ADMIN_PROJECTION_RUNTIME_ROLE}', relation.oid, 'USAGE,SELECT,UPDATE'
        ))
      ) AS "leastPrivilege",
      NOT EXISTS (SELECT 1 FROM foreign_relations WHERE
        (relkind IN ('r', 'p') AND (
          has_any_column_privilege(
            '${OPS_ADMIN_PROJECTION_RUNTIME_ROLE}', oid, 'SELECT,INSERT,UPDATE'
          ) OR
          has_table_privilege(
            '${OPS_ADMIN_PROJECTION_RUNTIME_ROLE}', oid, 'DELETE,TRUNCATE,REFERENCES,TRIGGER'
          )
        )) OR (relkind = 'S' AND has_sequence_privilege(
          '${OPS_ADMIN_PROJECTION_RUNTIME_ROLE}', oid, 'USAGE,SELECT,UPDATE'
        ))) AS "noCrossDomainAccess",
      NOT has_database_privilege(
        '${OPS_ADMIN_PROJECTION_RUNTIME_ROLE}', current_database(), 'CREATE'
      )
        AND NOT has_database_privilege(
          '${OPS_ADMIN_PROJECTION_RUNTIME_ROLE}', current_database(), 'TEMP'
        )
    ...
    if (
      ...
      throw new Error(
        'Ops/Admin projection runtime database role verification failed',
      );
    }
    ...
    return {
      ...
      relationCount: OPS_ADMIN_PROJECTION_RUNTIME_TABLES.length,
    };
  } catch (error) {
...
async function main(): Promise<void> {
  ...
  const password = process.env.OPS_ADMIN_PROJECTION_RUNTIME_PASSWORD;
  if (!ownerUrl || !password) {
    throw new Error(
      'OPS_ADMIN_PROJECTION_DATABASE_OWNER_URL and OPS_ADMIN_PROJECTION_RUNTIME_PASSWORD are required',
    );
  }
  ...
  try {
    const report = await provisionOpsAdminProjectionRuntimeRole(
      client,
      password,
      parsed.databaseName,
    );
...
if (require.main === module) {
  void main().catch(() => {
    process.stderr.write(
      'Ops/Admin projection runtime role provisioning failed\n',
    );
    process.exitCode = 1;
  });
}
