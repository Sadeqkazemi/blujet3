...
export type CoreOfferHoldVerification = { expectedTotalIrr: string };

@Injectable()
export class CoreOfferPricingService {
  constructor(
  ...
  async reprice(
    ...
    if (
      ...
      this.invalidOffer();
    if (payload.expiresAtMs <= Date.now()) this.expired('بازقیمت‌گذاری');
    const quote = await this.quotes.quote(dto);
    return {
      quote,
  ...
  verifyForHold(
    ...
    if (
      ...
      this.invalidOffer();
    if (payload.expiresAtMs <= Date.now()) this.expired('دریافت');
  ...
  private encode(payload: OfferTokenPayload, secret: string): string {
  ...
  private decode(token: string): OfferTokenPayload {
    ...
    if (!encoded || !supplied || rest.length > 0) this.invalidOffer();
    let payload: unknown;
    try {
  ...
  private expired(action: string): never {
    throw new ConflictException({
      code: ErrorCode.OFFER_EXPIRED,
      message: `پیشنهاد منقضی شده است و باید دوباره ${action} شود.`,
    });
  }
